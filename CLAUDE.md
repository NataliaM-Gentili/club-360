...
- QR check-in: each reservation's confirmation email includes a QR encoding `id_cliente:X|id_turno:Y|id_reserva:Z` (pipe-separated key:value pairs), scanned at `asistencia_bp`'s `/asistencia/registrar` endpoint. Generated by `enviar_comprobante_qr_turno()` (turno suelto) and `enviar_comprobantes_qr_clase()` (abono mensual) in `email_services.py`.

### Abonado vs No abonado

- **Abonado** (subscriber): reserves a full `Clase` via `ReservaClase` — pays monthly, can pay from the 1st to the 10th of the month. Gets one QR per turno of the month.
- **No abonado** (non-subscriber): reserves individual `Turno`s via `ReservaTurno` — must pay a 50% deposit (seña) upfront. Gets one QR per reservation.

### Precios por disciplina (`reserva_model.py`)
...
### Suspensiones (`ClienteSuspendido`)

- **Abonados**: suspended automatically on day 11 of each month if their `ReservaClase` is still `"Pendiente"` (`suspension_service.py → suspender_abonados_pendientes()`). Creates `ClienteSuspendido` with `id_clase`. Penalty: 5% surcharge on pro-rata amount (turnos del 1 al 10). The reservation is NOT deleted or cancelled — it stays `"Pendiente"`.
- **No abonados**: `cancelacion_model.py` has `supera_limite_sueltas()` (>3 unpaid in a month) but the call is **commented out** in `reserva_routes.py:636`. The check infrastructure exists (`ClienteSuspendido` with `id_turno`) and is enforced at reservation/waitlist time, but no code currently creates these suspensions automatically.
- **Removing suspensions**: employee pays via `POST /registrar_pago_suspension` (validates monto ≥ debt); client self-removes via `DELETE /pagar-suspension/<id>` (no payment validation).

### Cancelación de reservas (two distinct routes)

- **`DELETE /cancelar_reserva/<id>`** (`reserva_routes.py`): hard-deletes the `Reserva` and all related records (`ReservaTurno`, `Abono`, `AbonoTarjeta`). Only works on `"Pendiente"` reservations (e.g. failed payments).
- **`POST /cancelar_turno/<id>`** (`reserva_routes.py`): client-initiated cancellation. Sets `reserva.estado = "Cancelada"`, then calls `reset_reserva()` which recycles the reservation for the waitlist (`id_cliente = None`, `estado = "Pendiente"`, doubles `abono.monto`). Reintegro rules: ≥24h before → 50% refund; <24h → no refund; credit reservation → no refund.

### Asistencia (check-in)

- **Manual** (`POST /asistencia/registrar_manual`): employee enters client email + turno. Requires reservation in `"Pago"` state, within ±1 hour of class time.
- **QR** (`POST /asistencia/registrar`): employee scans QR. Same validation. Returns `PAGO_INCOMPLETO` for unpaid `ReservaTurno` with option to complete payment.
- Both insert into `cliente_asistio_turno` junction table and reject duplicates.

## Running load scripts

Scripts in `backend/instance/load_scripts/` need a `sys.path` fix because Python adds the script's directory to path, not `backend/`. Add this at the top of any new script:
...
Then run from `backend/` with venv activated:
```
python instance/load_scripts/script_name.py
```
